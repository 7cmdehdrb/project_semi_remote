...
class ManipulatorControl:
    def __init__(self):
        ...
        self.pub = rospy.Subscriber(
            "/servo_server/target_pose", PoseStamped, queue_size=10
        )
        ...
        rospy.Subscriber(
            "/tf_B2P_selected", Float32MultiArray, self.tf_B2P_selected_callback
        )
        self.target_reached_pub = rospy.Publisher(
            "/target_reached", Bool, queue_size=10
        )
        self.homepose_reached_pub = rospy.Publisher(
            "/homepose_reached", Bool, queue_size=10
        )
        ...
        self.manually_placing_pose = Pose()
        self.manually_placing_pose.position.x = -0.1
        self.manually_placing_pose.position.y = 0.85
        self.manually_placing_pose.position.z = 0.34
    ...
    def is_close(
        self, current_pose, target_pose, position_threshold, orientation_threshold=0.1
    ):
    ...
    def go_to_home_pose(self):
        ...
        # home_position = [-0.370, 0.133, 0.341]
    ...
    def while_running(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.target_bool:
                if self.is_close(
                    self.get_initial_pose(), self.target_pose, position_threshold=0.05
                ):
